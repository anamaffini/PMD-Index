...
For the final project of the NGEO306 PhD course I chose to develop a QGIS processing plugin that could calculate the PMD metrics that I have been developing with a colleague in my PhD.\
The proposed plugin is called PMD Index, which means Potential Movement Difference Index. To create the plugin, Plugin Builder in QGIS was used.\
The PMD Index plugin is constituted of a help folder, an icons folder, a pycache folder, an icon .png image, a metadata .txt file, a readme .md file, and four python files:
...
Because this report aims to present the main parts of the code that were written by me, it will focus mostly on the pmd algorithm script.\
The report is structure in three parts: this brief introduction; the PMD plugin, where the metrics and the plugin are presented; and the PMD algorithm, where the code is explained.

## 2. The PMD plugin

PMD Index is a set of urban network metrics that consider the urban space as a network graph for its analysis.\
The measure of Potential Movement (PM) is the frequency with which streets in the urban street network belong to the shortest paths of a population when heading to urban facilities, acting as an indicator of population flows.\
...
It calculates the differences in the Potential Movement values of specific population's groups when compared to a General value of Potential Movement.\
The PMD Index plugin calculates eight measures:

* The General Potential Movement (GPM)
* The Potential Movement for a Group of the Population (PMa)
* The Potential Movement for a Another Group of the Population (PMb)
...
The **PM** is a centrality measure that is restricted to ordered pairs of nodes (origins and destinations). Only the pairs with these opposite attributes are considered in the calculation.\
For the **origin** nodes, the PM uses the location of the households weighted by the amount of residents.\
...
The PM id defined by the equation:
$$
...
When the PM is calculated for the entire population of the city, it is called **General Potential Movement (GPM)**, and when it is calculated for a population group, it is called Potential Movement of group _x_ (PMx).


**Normalized Potential Movement**

After computing the GPM and the PM for the compared population groups, there is a normalization of the PM values. The equation of the normalized PM is:
$$
...
To calculate the PMD Index metrics, the PMD algorithm is structured in nine steps:

1. The initial imports
2. Definition of constants
3. Addition of parameters
4. Definition of the input parameters of the algorithm
...
First, the imports for the PMD algorithm:
...
Now, the definition of the constants:
...
We then start the algorithm by adding the parameters of the plugin.\
First, we add the parameter for the input  vector layer:
...
Next, we add the parameter for the analysis type.\
In here, there are two options (topological or geometric) that the user can chose from.
...
Next, we add the parameter for the rule for connecting lines.\
In here, there are three rules for how the algorithm is suposed to consider the connection of the lines. The the user must chose one.
...
Next, we add the parameter for the analysis radius.\
In here, if no number is typed by the user, the algorithm considers it as 0, and performs a global analysis.
...
Next, we add the impedance parameter.\
This parameter is an optional one, and can be selected by the user from one of the input vector field columns.
...
Next, we add the destination parameter.\
This parameter must be selected by the user from one of the input vector field columns.
...
Next, we add the origins of the entire population parameter.\
This parameter must be selected by the user from one of the input vector field columns.
...
Next, we add the origin of population group A parameter.\
This parameter must be selected by the user from one of the input vector field columns.
...
Next, we add the origin of population group B parameter.\
This parameter must be selected by the user from one of the input vector field columns.
...
Finally, we add the output vector parameter.
...
**INSERIR TODO O CODIGO AQUI!!!**

Next, we start the processing algorithm and define the input parameters.
...
We then start the Edge Object (edges of the network):
...
We define the metrics that will be calculated in the algorithm:
...
We define the destination, origins and impedance values, and how the algorithm should consider them.
...
Next we verify if highest id number is lower than number of features in the object in order to avoid potential conflicts with matrices' size.
...
Then, we start the edges processing:
...
Next, we compute the shortest paths using the djikstra algorithm with binary heap as priority queue.\
This process is structured in two parts: first the heap creation, then the heapsort.

For the heap creation:
...
Next, we beging the calculations for the metrics.\
We start with defining the the temporary variables for the metrics:
...
We need to sum the values of originG, originA, and originB for the gpmNorm, pmaNorm, and pmbNorm metrics.
...
We define the tension variables that will be used in the metrics.
...
We start the metrics calculations.\
First we calculate the PM values for the connected edges.
...
Next, we calculate the PM values for the source edges:
...
Then, we calculate the metrics for the edges in the shortest paths:
...
Finally, we update the metrics values:
...
After computing the metrics, we update the table of contents.\
To do so, we start creating new attributes fields for the computed metrics:
...
Then, we define the dictionary with the metrics variables and update the table of contents.
...
Finally, we save the output vector layer, which is a new vector layer, and delete the metrics attribute fields from the input vector layer.
